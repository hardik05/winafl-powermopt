...
#  define CASE_PREFIX "id_"
    ...
    if (!strncmp(rsl, CASE_PREFIX, 3) &&
      ...
      nfn = alloc_printf("%s\\queue\\id_%06u_orig_%s", out_dir, id, use_name);
...
static u8* describe_op(u8 hnb) {
  ...
  if (syncing_party) {
    ...
    sprintf(ret, "sync_%s_src_%06u", syncing_party, syncing_case);
    ...
    sprintf(ret + strlen(ret), "_op_%s", stage_short);

    if (stage_cur_byte >= 0) {

      sprintf(ret + strlen(ret), "_pos_%u", stage_cur_byte);

      if (stage_val_type != STAGE_VAL_NONE)
        sprintf(ret + strlen(ret), "_val_%s%+d", 
                (stage_val_type == STAGE_VAL_BE) ? "be_" : "",
                stage_cur_val);

    } else sprintf(ret + strlen(ret), "_rep_%u", stage_cur_val);

  }

  if (hnb == 2) strcat(ret, "_+cov");
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  switch (fault) {
    ...
    case FAULT_TMOUT:
      ...
      fn = alloc_printf("%s\\hangs\\id_%06llu_%s", out_dir,
    ...
    case FAULT_CRASH:
      ...
      fn = alloc_printf("%s\\crashes\\id_%06llu_sig_%02u_%s_%s", out_dir,
                        unique_crashes, kill_signal, describe_op(0), exception_name);
...
if (limit_time_sig == 1)
{
	if (key_puppet == 0)
	{
		if (key_module == 0)
		{
			sprintf(tmp + banner_pad, "WinAFL " WINAFL_VERSION " based on AFL " cLCY VERSION " %s" cLGN
